...
DATA_PATH = Path("/Users/allikhankoshamet/Desktop/projects/People_consulting/Copy_2026.xlsx")
...
def main() -> None:
    ...
    with tab4:
        ...
        if detail_rank.empty:
            ...
            # Аналитическая визуализация с прогресс-барами
            st.divider()
            st.markdown("#### 📊 Аналитика: сравнение со средними показателями")
            st.caption("Визуальное сравнение компании со средним по всему рынку и со средним по её сектору")
            ...
            # Создаём визуализацию для каждой метрики
            for mk in metrics_order:
                ...
                overall_pct = overall_mean * 100
                sector_pct = sector_mean * 100
                
                diff_overall = company_pct - overall_pct
                diff_sector = company_pct - sector_pct
                
                # Заголовок метрики
                st.markdown(f"### {METRIC_KIND_LABEL[mk]}")
                
                # Создаём данные для горизонтального графика
                comparison_data = pd.DataFrame([
                    {"category": "🏢 Компания", "value": company_pct, "type": "company"},
                    {"category": "📊 Среднее по рынку", "value": overall_pct, "type": "market"},
                    {"category": f"🏭 Среднее по сектору", "value": sector_pct, "type": "sector"},
                ])
                
                # Создаём горизонтальный bar chart
                bars = alt.Chart(comparison_data).mark_bar(cornerRadius=6, height=30).encode(
                    x=alt.X("value:Q", title="%", axis=alt.Axis(format=".1f")),
                    y=alt.Y("category:N", title="", sort=["🏢 Компания", "📊 Среднее по рынку", "🏭 Среднее по сектору"]),
                    color=alt.Color("type:N", scale=alt.Scale(
                        domain=["company", "market", "sector"],
                        range=["#3b82f6", "#94a3b8", "#8b5cf6"]
                    ), legend=None),
                    tooltip=["category", alt.Tooltip("value:Q", format=".1f")],
                )
                
                text = alt.Chart(comparison_data).mark_text(
                    align="left", dx=8, fontSize=13, color="#e2e8f0", fontWeight="bold"
                ).encode(
                    x=alt.X("value:Q"),
                    y=alt.Y("category:N", sort=["🏢 Компания", "📊 Среднее по рынку", "🏭 Среднее по сектору"]),
                    text=alt.Text("value:Q", format=".1f"),
                )
                
                chart = (bars + text).properties(height=130).configure_axis(
                    labelFontSize=12,
                    labelColor="#cbd5e1",
                    titleColor="#94a3b8"
                )
                ...
                # Показываем разницы в виде метрик
                col_diff1, col_diff2 = st.columns(2)
                
                with col_diff1:
                    delta_color = "normal" if diff_overall >= 0 else "inverse"
                    st.metric(
                        label="📊 Разница от рынка",
                        value=f"{diff_overall:+.1f} п.п.",
                        delta=f"{'выше' if diff_overall > 0 else 'ниже' if diff_overall < 0 else 'равно'} среднего по рынку",
                ...
                with col_diff2:
                    ...
                    st.metric(
                        label=f"🏭 Разница от сектора «{company_sector}»",
                        value=f"{diff_sector:+.1f} п.п.",
                        delta=f"{'выше' if diff_sector > 0 else 'ниже' if diff_sector < 0 else 'равно'} среднего по сектору",
                        delta_color=delta_color
                    )
            ...
            **Как читать визуализацию:**
            - 🔵 Синяя полоса — показатель компании
            - ⚫ Серая полоса — среднее по всему рынку
            - 🟣 Фиолетовая полоса — среднее по сектору компании
            - 📈 Положительная разница (зелёная) = компания выше среднего
            - 📉 Отрицательная разница (красная) = компания ниже среднего
            """)
    ...
    with tab7:
        ...
        st.caption("Как работники отрасли оценивают компании своей же отрасли — взгляд изнутри")

        # Используем сектора компаний как основу
        available_sectors_expert = sorted(data.companies["sector"].dropna().unique().tolist())
        
        if not available_sectors_expert:
            st.warning("Нет данных по секторам")
        else:
            selected_expert_sector = st.selectbox(
                "🏢 Выберите сектор для экспертного анализа",
                options=available_sectors_expert,
                key="expert_sector_select",
                help="Показывает как работники этого сектора оценивают компании своего же сектора"
            )

            # Проверяем есть ли колонка "В какой отрасли"
            if "В какой отрасли" not in respondents.columns:
                st.warning("В данных нет информации об отрасли респондентов ('В какой отрасли')")
            else:
                # Фильтруем респондентов, которые работают в выбранном секторе
                # Нужно найти соответствие между "В какой отрасли" и sector
                sector_respondents = respondents[
                    respondents["В какой отрасли"].str.contains(selected_expert_sector, case=False, na=False) |
                    (respondents["В какой отрасли"] == selected_expert_sector)
                ].copy()
                
                sector_ids = set(sector_respondents["respondent_id"].dropna().astype(int).tolist())
                
                st.markdown(f"**Респондентов работающих в секторе «{selected_expert_sector}»:** {len(sector_respondents)}")

                if len(sector_ids) == 0:
                    st.info(f"Нет респондентов работающих в секторе «{selected_expert_sector}». Попробуйте другой сектор.")
                    
                    # Показываем какие отрасли есть у респондентов
                    st.markdown("**Доступные отрасли респондентов:**")
                    available_resp_sectors = respondents["В какой отрасли"].dropna().unique().tolist()
                    st.write(", ".join(sorted(available_resp_sectors)[:10]))
                else:
                    # Получаем ответы этих респондентов
                    sector_company_data = data.company_raw_long[data.company_raw_long["respondent_id"].isin(sector_ids)].copy()
                    
                    # Фильтруем только компании из того же сектора
                    sector_companies = data.companies[data.companies["sector"] == selected_expert_sector]["company_key"].tolist()
                    
                    if not sector_companies:
                        st.info(f"Нет компаний в секторе «{selected_expert_sector}»")
                    else:
                        expert_data = sector_company_data[sector_company_data["company_key"].isin(sector_companies)].copy()
                        
                        if expert_data.empty:
                            st.info("Эксперты этого сектора не оценивали компании своего сектора")
                        else:
                            # Вычисляем метрики как эксперты оценивают свои компании
                            expert_metrics = compute_company_metrics_for_segment(expert_data, len(sector_ids))
                            
                            if expert_metrics.empty:
                                st.info("Недостаточно данных для анализа")
                            else:
                                st.success(f"✅ Найдено {len(expert_data)} оценок от экспертов сектора «{selected_expert_sector}» для компаний этого же сектора")
                                
                                st.markdown("#### 📊 Топ компаний по оценке экспертов отрасли")
                                
                                top_n_expert = st.slider("Количество компаний", 5, 15, 10, key="expert_top")
                                
                                c1, c2 = st.columns(2)
                                with c1:
                                    expert_top_want = expert_metrics.sort_values("want_pct", ascending=False).head(top_n_expert).copy()
                                    expert_top_want["pct"] = (expert_top_want["want_pct"] * 100).round(1)
                                    expert_top_want["label"] = expert_top_want["pct"].map(lambda x: f"{x:.1f}%")
                                    st.altair_chart(
                                        _bar_pct_horizontal(expert_top_want, "pct", "company", f"✅ Хотят работать (эксперты {selected_expert_sector})"),
                                        use_container_width=True
                                    )
                                
                                with c2:
                                    expert_anti_top = expert_metrics.sort_values("dont_want_pct", ascending=False).head(top_n_expert).copy()
                                    expert_anti_top["pct"] = (expert_anti_top["dont_want_pct"] * 100).round(1)
                                    expert_anti_top["label"] = expert_anti_top["pct"].map(lambda x: f"{x:.1f}%")
                                    st.altair_chart(
                                        _bar_pct_horizontal(expert_anti_top, "pct", "company", f"❌ Не хотят работать (эксперты {selected_expert_sector})"),
                                        use_container_width=True
                                    )
                                
                                st.divider()
                                st.markdown("#### 📋 Полная таблица экспертных оценок")
                                
                                expert_table = expert_metrics[["company", "sector", "want_pct", "dont_want_pct", "uncertain_pct", "unknown_brand_pct"]].copy()
                                expert_table["want_pct"] = (expert_table["want_pct"] * 100).round(1)
                                expert_table["dont_want_pct"] = (expert_table["dont_want_pct"] * 100).round(1)
                                expert_table["uncertain_pct"] = (expert_table["uncertain_pct"] * 100).round(1)
                                expert_table["unknown_brand_pct"] = (expert_table["unknown_brand_pct"] * 100).round(1)
                                expert_table = expert_table.rename(columns={
                                    "company": "Компания",
                                    "sector": "Сектор",
                                    "want_pct": "Хотят, %",
                                    "dont_want_pct": "Не хотят, %",
                                    "uncertain_pct": "Не уверены, %",
                                    "unknown_brand_pct": "Не знают, %",
                                })
                                st.dataframe(expert_table.sort_values("Хотят, %", ascending=False), use_container_width=True, hide_index=True)
